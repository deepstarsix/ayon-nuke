...
class ExtractReviewIntermediates(publish.Extractor):
    ...
    order = pyblish.api.ExtractorOrder + 0.012
    ...
    def process(self, instance):
        ...
        with maintained_selection():
            ...
            for o_data in self.outputs:
                ...
                # --- PATCH FOR SLATE COMPATIBILITY ---
                # Ensure slate extractor downstream can process this representation
                for repre in data["representations"]:
                    # Add the "slate-frame" tag if not present
                    tags = repre.get("tags", [])
                    if "slate-frame" not in tags:
                        tags.append("slate-frame")
                    repre["tags"] = tags

                    # Add outputDef for slate extractor
                    if "outputDef" not in repre and "outputDef" in o_data:
                        repre["outputDef"] = o_data["outputDef"]
                    elif "outputDef" not in repre:
                        repre["outputDef"] = o_data
